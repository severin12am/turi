...
const DialogueBox: React.FC<DialogueBoxProps> = ({
  ...
  const showQuizAfterDialogue = (dialogueId: number) => {
    ...
    // First check if this is the first dialogue and user is not logged in
    // In that case, skip quiz and directly show signup prompt
    const { user, isLoggedIn } = useStore.getState();
    if (dialogueId === 1 && (!isLoggedIn || !user)) {
      console.log("🎲 First dialogue complete, showing signup prompt directly");
      
      // Cleanup speech components
      try {
        if (recognitionRef.current) {
          try {
            recognitionRef.current.abort();
        ...
        console.error("Error during cleanup for signup prompt:", e);
      }
      
      // Show signup prompt instead of quiz
      setTimeout(() => {
        setShowSignupPrompt(true);
        console.log("🎲 Signup prompt set to visible");
      }, 500);
  ...
  const handleQuizComplete = (passed: boolean) => {
    ...
    // If this is the first dialogue and user isn't logged in, show signup prompt
    if (currentDialogueId === 1 && (!user || !isLoggedIn)) {
      console.log('DialogueBox - First dialogue completed, user not logged in, showing signup prompt');
      setShowQuiz(false);
      setTimeout(() => {
        setShowSignupPrompt(true);
      }, 500);
    ...
    if (passed && user?.id && isLoggedIn) {
      const updateProgress = async () => {
        try {
          ...
          // Manually force a refresh of the language level data
          // This ensures the next dialogue will be unlocked immediately
          try {
            ...
            console.log('DialogueBox - Refreshed language level data after quiz completion:', refreshedData);
          } catch (refreshError) {
            console.error('DialogueBox - Error refreshing language data:', refreshError);
          }
        } catch (error) {
