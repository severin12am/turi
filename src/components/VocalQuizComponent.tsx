...
const translations: Record<string, Record<Lang, string>> = {
  ...
  'Let\'s try again!': { en: 'Let\'s try again!', ru: 'Давайте попробуем еще раз!' },
  ...
  'Turi believes in you! A little more practice and you\'ll master these words.': { 
    ...
    ru: 'Тури верит в вас! Еще немного практики, и вы освоите эти слова.' 
  }
};
    ...
    try {
      ...
      if (user?.id) {
        try {
          ...
          // IMPORTANT: Explicitly refresh language level data after completion
          // This ensures the DialogueSelectionPanel will have the latest data
          try {
            console.log("VocalQuizComponent - Explicitly refreshing language level data to ensure next dialogue is unlocked");
            
            // Force a refresh by directly querying the database
            const { data: refreshedLevelData, error: refreshError } = await supabase
            ...
            if (refreshError) {
              console.error("VocalQuizComponent - Error refreshing language level data:", refreshError);
            } else {
              console.log("VocalQuizComponent - Successfully refreshed language level data:", refreshedLevelData);
            }
          } catch (refreshError) {
            console.error("VocalQuizComponent - Exception during language level refresh:", refreshError);
          }
        ...
        <div className="w-full max-w-md p-8 mx-4 shadow-2xl rounded-xl bg-slate-900/80 backdrop-blur-md border border-slate-700 text-white pointer-events-auto">
          <div className="flex flex-col items-center justify-center space-y-6">
            ...
            <h2 className="text-3xl font-bold text-white">
              {passed ? t("Great work!", motherLanguage) : t("Let's try again!", motherLanguage)}
            </h2>
            ...
            <p className="text-lg text-slate-200 text-center bg-indigo-900/20 p-4 rounded-lg border border-indigo-800/40">
              {passed 
                ? t('Turi is impressed with your progress! You\'re doing great with your language journey.', motherLanguage) 
                : t('Turi believes in you! A little more practice and you\'ll master these words.', motherLanguage)}
            </p>
