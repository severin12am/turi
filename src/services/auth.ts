...
export const initializeLanguageLevel = async (userId: string, language: string, motherLanguage: string) => {
  try {
    ...
    // First, check if a language level already exists for this user and language pair
    const { data: existingLevel, error: levelCheckError } = await supabase
    ...
    // If language level already exists for this language pair, return it
    if (!levelCheckError && existingLevel) {
      console.log('💬 Language level already exists for this language pair:', existingLevel);
      logger.info('Language level already exists for this language pair', { 
        userId, 
        language, 
        motherLanguage, 
    ...
    if (userCheckError && userCheckError.code === 'PGRST116') {
      ...
      console.log('💬 Creating language level record with:', { wordCount, level: 1, dialogue_number: 0 });
      const { data, error } = await supabase
        .from('language_levels')
        .insert([{
          user_id: userId,
          ...
          mother_language: motherLanguage,
          level: 1,
          word_progress: 0,         // Start with 0 progress
          dialogue_number: 0         // Start with 0 dialogues completed
        }])
        .select();
        ...
        logger.info('Language level initialized', { userId, language, wordProgress: 0, dialogueNumber: 0 });
        return data;
    } catch (error) {
...
export const updateWordProgress = async (
  ...
  try {
    ...
    if (levelError && levelError.code === 'PGRST116') {
      ...
      console.log('💬 Creating language level record with:', { wordCount, level, dialogue_number: 0 });
...
export const getCompletedDialogues = async (userId: string) => {
  try {
    ...
    // Initialize empty array for completed dialogues
    let completedDialogues: DialogueProgress[] = [];
    
    // Get the user's language level which contains their dialogue progress
    ...
    // Use dialogue_number to determine completed dialogues
    if (languageLevel && languageLevel.dialogue_number > 0) {
      console.log('💬 Creating completed dialogues from dialogue_number:', languageLevel.dialogue_number);
